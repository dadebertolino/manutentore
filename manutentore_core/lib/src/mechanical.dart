...
class CoppiaSerraggio extends Calculator {
  ...
  @override
  List<String> get references => ['ISO 898-1', 'VDI 2230 (metodo semplificato)'];
  @override
  String? get theory =>
  ...
  List<FieldSpec> get fields => const [
        FieldSpec.number('d', 'Diametro nominale',
            unit: 'mm', min: 3, max: 48, value: 12),
        FieldSpec.number('passo', 'Passo',
            unit: 'mm', min: 0, max: 6, value: 0,
            help: '0 = passo grosso da tabella ISO 261.'),
        FieldSpec.select('classe', 'Classe di resistenza', options: [
        ...
        FieldSpec.select('attrito', 'Condizione di attrito', options: [
          SelectOption('asciutto', 'Asciutto, non trattato (K=0,20)',
              numeric: 0.20),
          ...
          SelectOption('mos2', 'MoS2 / pasta antigrippante (K=0,12)',
              numeric: 0.12),
        ], value: 'asciutto'),
        FieldSpec.number('sfrutt', 'Sfruttamento del carico di snervamento',
  ...
  CalcResult compute(Inputs i) {
    ...
    if (passo <= 0) {
      passo = interpola(
          kPassoGrosso.map((k, v) => MapEntry(k as num, v)), d);
    ...
    return CalcResult(
      [
        ResultLine.number('Coppia di serraggio', coppia, unit: 'Nm',
            decimals: 1, primary: true),
        ResultLine.number('Forza di precarico', precarico / 1000, unit: 'kN',
            decimals: 2, primary: true),
        ResultLine.number('Sezione resistente As', as_, unit: 'mm2',
            decimals: 2),
        ResultLine.number('Passo utilizzato', passo, unit: 'mm'),
        ResultLine.number('Tensione nel gambo', sigma, unit: 'MPa',
            decimals: 0, note: 'Snervamento classe: ${rp.toStringAsFixed(0)} MPa'),
        ResultLine.number('Carico di rottura teorico', caricoRottura,
            unit: 'kN', decimals: 1),
        ResultLine.number('Coppia con +20% attrito', coppia * 1.2, unit: 'Nm',
            decimals: 1, severity: Severity.warn,
            note: 'Dispersione tipica del serraggio a chiave dinamometrica'),
      ],
      verdict: 'M${d.toStringAsFixed(0)}x$passo classe ${i.opt('classe')}: '
...
class ForoMaschiatura extends Calculator {
  ...
  List<FieldSpec> get fields => const [
        FieldSpec.number('d', 'Diametro nominale',
            unit: 'mm', min: 2, max: 48, value: 12),
        FieldSpec.number('passo', 'Passo',
            unit: 'mm', min: 0, max: 6, value: 0,
            help: '0 = passo grosso da tabella.'),
        FieldSpec.number('foro', 'Diametro punta effettivo',
            unit: 'mm', min: 0, max: 48, value: 0,
  ...
  CalcResult compute(Inputs i) {
    ...
    final as_ = 0.7854 * math.pow(d - 0.9382 * passo, 2).toDouble();
    final foroPassante = d + (d <= 6 ? 0.4 : (d <= 20 ? 1.0 : 2.0)); // serie media
    ...
    return CalcResult(
      [
        ResultLine.number('Punta consigliata', consigliato, unit: 'mm',
            primary: true),
        ResultLine.number('Percentuale di filetto', percFiletto, unit: '%',
            decimals: 1, primary: true, severity: sev, note: nota),
        ResultLine.number('Passo', passo, unit: 'mm'),
        ResultLine.number('Diametro medio d2', d2, unit: 'mm', decimals: 3),
        ResultLine.number('Diametro di nocciolo d3', d3, unit: 'mm',
            decimals: 3),
        ResultLine.number('Diametro minore filetto interno', d1int, unit: 'mm',
            decimals: 3),
        ResultLine.number('Sezione resistente As', as_, unit: 'mm2',
            decimals: 2),
        ResultLine.number('Foro passante (serie media)', foroPassante,
            unit: 'mm', decimals: 1),
      ],
      verdict: 'M${d.toStringAsFixed(0)}x$passo -> punta '
...
class VitaCuscinetto extends Calculator {
  ...
  @override
  List<FieldSpec> get fields => const [
        FieldSpec.number('c', 'Coefficiente di carico dinamico C',
            unit: 'kN', min: 0.01, max: 5000, value: 14.0),
        FieldSpec.number('p', 'Carico dinamico equivalente P',
            unit: 'kN', min: 0.001, max: 5000, value: 2.5),
        FieldSpec.number('n', 'Velocita\' di rotazione',
            unit: 'rpm', min: 0.1, max: 50000, value: 1450),
        FieldSpec.select('tipo', 'Tipo di corpo volvente', options: [
          SelectOption('sfere', 'Sfere (p=3)', numeric: 3),
          SelectOption('rulli', 'Rulli (p=10/3)', numeric: 3.3333333333333335),
        ], value: 'sfere'),
        FieldSpec.number('a1', 'Fattore di affidabilita\' a1',
            min: 0.05, max: 1, value: 1,
            help: '1 = 90% (L10). 0,62 = 95%. 0,53 = 96%. 0,33 = 98%.'),
        FieldSpec.number('aiso', 'Fattore lubrificazione/contaminazione',
            min: 0.05, max: 50, value: 1,
  ...
  CalcResult compute(Inputs i) {
    ...
    return CalcResult(
      [
        ResultLine.number('Vita nominale L10h', l10h, unit: 'h',
            decimals: 0, primary: true),
        ResultLine.number('Vita corretta Lnmh', lnmh, unit: 'h',
            decimals: 0, primary: true),
        ResultLine.number('L10 in milioni di giri', l10, decimals: 1),
        ResultLine.number('Rapporto C/P', rapporto, decimals: 2,
            severity: sev,
            note: sev == Severity.warn
                ? 'C/P basso: cuscinetto molto caricato'
                : null),
        ResultLine.number('Durata in anni continui', anni, decimals: 2),
        ResultLine.number('C equivalente richiesto', cRichiesto, unit: 'kN',
            decimals: 2),
      ],
      verdict: lnmh >= 20000
...
class FrequenzeCuscinetto extends Calculator {
  ...
  Domain get domain => Domain.meccanico;
  @override
  List<String> get tags => ['bpfo', 'bpfi', 'bsf', 'ftf', 'vibrazioni', 'spettro'];
  @override
  ...
  @override
  List<FieldSpec> get fields => const [
        FieldSpec.number('n', 'Velocita\' albero',
            unit: 'rpm', min: 1, max: 30000, value: 1450),
        FieldSpec.number('nb', 'Numero di corpi volventi',
            min: 3, max: 60, value: 9),
        FieldSpec.number('d', 'Diametro corpo volvente',
            unit: 'mm', min: 0.5, max: 200, value: 7.94),
        FieldSpec.number('dp', 'Diametro primitivo',
            unit: 'mm', min: 2, max: 2000, value: 39.04,
            help: 'Media tra diametro interno ed esterno se non noto.'),
        FieldSpec.number('alfa', 'Angolo di contatto',
            unit: 'gradi', min: 0, max: 45, value: 0),
      ];
  ...
  CalcResult compute(Inputs i) {
    ...
    return CalcResult(
      [
        ResultLine.number('Frequenza di rotazione', fr, unit: 'Hz',
            decimals: 3, note: '1x'),
        ResultLine.number('BPFO - pista esterna', bpfo, unit: 'Hz',
            decimals: 2, primary: true,
            note: '${(bpfo / fr).toStringAsFixed(2)}x'),
        ResultLine.number('BPFI - pista interna', bpfi, unit: 'Hz',
            decimals: 2, primary: true,
            note: '${(bpfi / fr).toStringAsFixed(2)}x'),
        ResultLine.number('BSF - corpo volvente', bsf, unit: 'Hz',
            decimals: 2, note: '${(bsf / fr).toStringAsFixed(2)}x'),
        ResultLine.number('FTF - gabbia', ftf, unit: 'Hz',
            decimals: 3, note: '${(ftf / fr).toStringAsFixed(3)}x'),
        ResultLine.number('2x BSF (urto doppio)', 2 * bsf, unit: 'Hz',
            decimals: 2),
        ResultLine.number('Frequenza massima consigliata analisi',
            math.max(bpfi * 3.5, 10 * fr),
            unit: 'Hz', decimals: 0,
            note: 'Impostare Fmax dello strumento almeno a questo valore'),
      ],
      verdict: 'Difetto interno atteso a ${bpfi.toStringAsFixed(1)} Hz, '
...
class SeveritaVibrazioni extends Calculator {
  ...
  @override
  List<FieldSpec> get fields => const [
        FieldSpec.number('v', 'Velocita\' efficace misurata',
            unit: 'mm/s RMS', min: 0, max: 100, value: 3.2,
            help: 'Banda 10-1000 Hz.'),
        FieldSpec.select('gruppo', 'Taglia macchina', options: [
          SelectOption('G1', 'Gruppo 1: oltre 300 kW'),
          SelectOption('G2', 'Gruppo 2: da 15 a 300 kW'),
        ], value: 'G2'),
        FieldSpec.select('supporto', 'Tipo di supporto', options: [
  ...
  CalcResult compute(Inputs i) {
    ...
    return CalcResult(
      [
        ...
        ResultLine.number('Valore misurato', v, unit: 'mm/s RMS',
            primary: true, severity: sev),
        ...
        ResultLine.number('Margine sul limite C/D',
...
class CilindroPneumatico extends Calculator {
  ...
  @override
  List<FieldSpec> get fields => const [
        FieldSpec.number('alesaggio', 'Alesaggio',
            unit: 'mm', min: 4, max: 500, value: 50),
        FieldSpec.number('stelo', 'Diametro stelo',
            unit: 'mm', min: 2, max: 250, value: 20),
        FieldSpec.number('corsa', 'Corsa',
            unit: 'mm', min: 1, max: 5000, value: 200),
        FieldSpec.number('p', 'Pressione relativa',
            unit: 'bar', min: 0.5, max: 20, value: 6),
        FieldSpec.number('rend', 'Rendimento',
            min: 0.5, max: 1, value: 0.9,
            help: 'Attriti di tenuta: tipico 0,85-0,95.'),
        FieldSpec.number('cicli', 'Cicli al minuto',
            min: 0, max: 600, value: 10),
      ];
  ...
  CalcResult compute(Inputs i) {
    ...
    return CalcResult(
      [
        ResultLine.number('Forza in spinta', fSpinta, unit: 'N',
            decimals: 0, primary: true),
        ResultLine.number('Forza in tiro', fTiro, unit: 'N',
            decimals: 0, primary: true),
        ResultLine.number('Massa sollevabile in spinta', fSpinta / 9.81,
            unit: 'kg', decimals: 1),
        ResultLine.number('Area di spinta', aSpinta, unit: 'mm2', decimals: 1),
        ResultLine.number('Area di tiro', aTiro, unit: 'mm2', decimals: 1),
        ResultLine.number('Rapporto tiro/spinta', fTiro / fSpinta,
            decimals: 3),
        ResultLine.number('Consumo per ciclo', volCiclo, unit: 'Nl',
            decimals: 3),
        ResultLine.number('Portata richiesta', portata, unit: 'Nl/min',
            decimals: 1, primary: true),
      ],
...
class CoefficienteKv extends Calculator {
  ...
  @override
  List<FieldSpec> get fields => const [
        FieldSpec.select('verso', 'Cosa calcolare', options: [
          SelectOption('kv', 'Kv da portata e perdita'),
          SelectOption('q', 'Portata da Kv e perdita'),
          SelectOption('dp', 'Perdita da Kv e portata'),
        ], value: 'kv'),
        FieldSpec.number('q', 'Portata',
            unit: 'm3/h', min: 0, max: 100000, value: 15),
        FieldSpec.number('dp', 'Perdita di carico',
            unit: 'bar', min: 0, max: 500, value: 0.8),
        FieldSpec.number('kv', 'Kv valvola',
            unit: 'm3/h', min: 0, max: 100000, value: 16.8),
        FieldSpec.number('densita', 'Densita\' relativa',
  ...
  CalcResult compute(Inputs i) {
    ...
    return CalcResult(
      [
        ...
        ResultLine.number('Perdita in colonna d\'acqua', dpMetri, unit: 'm',
            decimals: 2),
      ],
...
class TrasmissionePulegge extends Calculator {
  ...
  @override
  List<FieldSpec> get fields => const [
        FieldSpec.number('n1', 'Velocita\' motrice',
            unit: 'rpm', min: 0.1, max: 30000, value: 1450),
        FieldSpec.number('d1', 'Diametro puleggia motrice',
            unit: 'mm', min: 5, max: 3000, value: 100),
        FieldSpec.number('d2', 'Diametro puleggia condotta',
            unit: 'mm', min: 5, max: 5000, value: 250),
        FieldSpec.number('interasse', 'Interasse',
            unit: 'mm', min: 10, max: 20000, value: 600),
        FieldSpec.number('p', 'Potenza trasmessa',
            unit: 'kW', min: 0, max: 5000, value: 7.5),
      ];
  ...
  CalcResult compute(Inputs i) {
    ...
    final abbraccio =
        180 - 2 * math.asin((d2 - d1) / (2 * c)) * 180 / math.pi;
    ...
    return CalcResult(
      [
        ResultLine.number('Rapporto di trasmissione', rapporto,
            decimals: 3, primary: true),
        ResultLine.number('Velocita\' condotta', n2, unit: 'rpm',
            decimals: 1, primary: true),
        ResultLine.number('Velocita\' periferica', v, unit: 'm/s',
            severity: veloce ? Severity.warn : Severity.ok),
        ResultLine.number('Lunghezza primitiva cinghia', lung, unit: 'mm',
            decimals: 0),
        ResultLine.number('Angolo di avvolgimento sulla piccola', abbraccio,
            unit: 'gradi',
            decimals: 1,
      ...
      verdict: 'Riduzione ${rapporto.toStringAsFixed(2)}:1 -> '
          '${n2.toStringAsFixed(0)} rpm',
      verdictSeverity:
          (veloce || abbraccioBasso) ? Severity.warn : Severity.ok,
      warnings: [
        if (abbraccioBasso)
          'Avvolgimento sotto 120 gradi: rischio di slittamento, aumentare '
              'l\'interasse o usare un rullo tenditore.',
        if (veloce)
          'Oltre 30 m/s servono pulegge equilibrate dinamicamente.',
      ],
    );
  }
}
...
class PotenzaCoppiaVelocita extends Calculator {
  ...
  @override
  List<FieldSpec> get fields => const [
        FieldSpec.select('incognita', 'Cosa calcolare', options: [
          SelectOption('c', 'Coppia'),
          SelectOption('p', 'Potenza'),
          SelectOption('n', 'Velocita\''),
        ], value: 'c'),
        FieldSpec.number('p', 'Potenza',
            unit: 'kW', min: 0, max: 100000, value: 7.5),
        FieldSpec.number('n', 'Velocita\'',
            unit: 'rpm', min: 0, max: 100000, value: 1440),
        FieldSpec.number('c', 'Coppia',
            unit: 'Nm', min: 0, max: 1000000, value: 49.7),
        FieldSpec.number('rend', 'Rendimento della catena cinematica',
  ...
  CalcResult compute(Inputs i) {
    ...
    return CalcResult(
      [
        ...
        ResultLine.number('Potenza', p, unit: 'kW', decimals: 3,
            primary: true),
        ResultLine.number('Velocita\'', n, unit: 'rpm', decimals: 1,
            primary: true),
        ResultLine.number('Velocita\' angolare', omega, unit: 'rad/s',
            decimals: 3),
        ResultLine.number('Potenza in cavalli', p * 1.35962, unit: 'CV'),
        if (eta < 1)
          ResultLine.number('Potenza richiesta a monte', pAlbero, unit: 'kW',
              decimals: 3),
      ],
      verdictSeverity: Severity.ok,
    );
